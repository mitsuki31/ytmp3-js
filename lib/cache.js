...
class VInfoCache {
  ...
  static async getCache(id, cacheOptions={}) {
    ...
    const decodeInfo = ['type', 'data'].every(key => key in cache.videoInfo)
      ...
      : null;
    cache = { ...cache, videoInfo: decodeInfo };
   ...
   *
   * If `humanReadable` option is enabled, the function formats the cache
   * contents into a simple human-readable string.
   *
   * @param {string} cachePath - The path to the cache directory, defaults to
   *                             {@link module:cache~VINFO_CACHE_PATH `VINFO_CACHE_PATH`} if not provided.
   * @param {boolean} humanReadable - Whether to format the cache into a human-readable string. Can be useful
   *                                  for checking the stored caches information.
   *
   ...
   *          if the `humanReadable` option is enabled, or an empty array if no caches are found.
   *
   ...
   * @since   2.0.0
   */
  static async getAllCaches(cachePath, humanReadable=false) {
    if (typeof cachePath !== 'string' && !TypeUtils.isNullOrUndefined(cachePath)) {
    ...
    if (humanReadable) {
      ...
      for (const cache of cacheList) {
        cacheStr += await VInfoCache.getCache(path.basename(cache), {
          cachePath,
          humanReadable: true
        });
