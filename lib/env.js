 ...
 *
 * @returns {boolean} `true` if the global interruption symbol exists and its `'interrupted'` property is `true`;
 *                    otherwise, `false`.
 * @package
 * @since   2.0.0
 */
function hasInterrupted() {
  return hasSetup() && getGlob('interrupted', false) === true;
}

/**
 * Sets the `'interrupted'` flag to `true` in the global `YTMP3_SYMBOL` object,
...
function setInterrupted() {
  if (hasSetup() && !hasInterrupted()) {
    global[YTMP3_SYMBOL]['interrupted'] = true;
  }
}
