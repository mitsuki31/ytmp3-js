 ...
 *          configuration file, or `null` if no global configuration file is found.
 *
 * @async
 * @package
 * @since    1.1.0
...
async function findGlobalConfig() {
  const configDirname = YTMP3_HOMEDIR;
  const knownConfigExtsRegex = new RegExp(`${KNOWN_CONFIG_EXTS.join('$|')}$`);
  const configs = await ls(configDirname, {
    encoding: 'utf8',
  ...
  if (configs.length === 1) return path.join(configDirname, configs[0]);
  ...
  let configRealPath = path.join(configDirname, configs[configIndex]);
  ...
  while (!configStat.size) {
    ...
    configRealPath = path.join(configDirname, configs[retries++]);
