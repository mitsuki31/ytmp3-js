...
/**
 * A function to create a line with customizable width to specific log stream.
 *
 * If the provided writable stream is representing a file (instead of a terminal TTY), the
...
Logger.line = function line(width, stream) {
  stream = stream instanceof Writable ? stream : process.stdout;
  ...
  Logger.info('-'.repeat(hasColumns ? ((len >= 75) ? (len / 2 + 15) : (len - 8)) : len));
};
 ...
 * @returns {Logger} A logger object with log methods disabled according to the specified log level.
