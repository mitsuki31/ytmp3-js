...
Logger.isStdStream = function isStdStream(stream, fd) {
  return stream instanceof Writable && !(fd < 1 || fd > 2)
    && ((typeof fd === 'number') ? (stream.fd === fd) : (
      (stream.fd === 1) || (stream.fd === 2)
    ));
};
...
Logger.info = function info(...msg) {
  ...
  const isStd = Logger.isStdStream(stream);
  ...
  prefix = ((!isStd || stream?.columns >= 75) ? prefix : '') + Logger.INFO_PREFIX + ' ';
  return stream.write(Logger.stripIfNotTTY(prefix + msg.join(' '), stream) + '\n');
};
...
Logger.done = function done(...msg) {
  ...
  let prefix = style([0, '~'], `${customDateFormat(new Date())}::`);
  prefix = ((!isStd || stream?.columns >= 75) ? prefix : '') + Logger.DONE_PREFIX + ' ';
  return stream.write(Logger.stripIfNotTTY(prefix + msg.join(' '), stream) + '\n');
};
...
Logger.debug = function debug(...msg) {
  ...
  let prefix = style([0, '~'], `${customDateFormat(new Date())}::`);
  prefix = ((!isStd || stream?.columns >= 75) ? prefix : '') + Logger.DEBUG_PREFIX + ' ';
  return stream.write(Logger.stripIfNotTTY(prefix + msg.join(' '), stream) + '\n');
};
...
Logger.warn = function warn(...msg) {
  ...
  let prefix = style([0, '~'], `${customDateFormat(new Date())}::`);
  prefix = ((!isStd || stream?.columns >= 75) ? prefix : '') + Logger.WARNING_PREFIX + ' ';
  return stream.write(Logger.stripIfNotTTY(prefix + msg.join(' '), stream) + '\n');
};
...
Logger.error = function error(...msg) {
  ...
  let prefix = style([0, '~'], `${customDateFormat(new Date())}::`);
  prefix = ((!isStd || stream?.columns >= 75) ? prefix : '') + Logger.ERROR_PREFIX + ' ';
  return stream.write(Logger.stripIfNotTTY(prefix + msg.join(' '), stream) + '\n');
};
...
Logger.line = function line(width, stream) {
  ...
  const len = hasColumns ? stream.columns : (typeof width === 'number' ? width : 80);
  return stream.write('-'.repeat(
    hasColumns ? ((len >= 75) ? (len / 2 + 15) : (len - 8)) : len) + '\n');
};
...
Logger.createLogger = function createLogger(logLevel, options) {
  ...
  if (logLevel === NONE) {
    return ['info', 'done', 'debug', 'warn', 'error'].reduce((acc, val) => {
