 ...
 * @property {?string} id - The ID of the thumbnail, typically used to differentiate resolution types (e.g., 'hqdefault', 'mqdefault').
 * @global
 * @since 2.0.0
 */
...
class DateFormatter {
  ...
  /**
   * Parse a timestamp in microseconds and return a ` instance.
   *
  ...
  static fromMicroseconds(microsecs) {
    ...
    return new DateFormatter(Math.floor(microsecs / 10e2));  // Multiply by 1000 to convert to milliseconds
  }
...
/**
 * Retrieves all thumbnails for a video, sorted by resolution from low to high.
 *
 * @param {Object} videoDetails - The `videoDetails` object from `ytdl.getInfo()`.
 * @returns {Array.<ThumbnailObject>} Sorted list of thumbnail objects from low to high resolution,
 *                                    or an empty list if thumbnails cannot be found.
 *
 * @throws {InvalidTypeError} If the given `videoDetails` is not an object type.
 *
 * @memberof module:utils/format-utils~FormatUtils
 * @public
 * @since 2.0.0
 */
function getAllThumbnails(videoDetails) {
  if (!TypeUtils.isObject(videoDetails)) {
    throw new InvalidTypeError('Invalid `videoDetails` object type', {
      actualType: TypeUtils.getType(videoDetails),
      expectedType: '[object Any]'
    });
  }

  // Return early when thumbnails cannot be found
  if (!Array.isArray(videoDetails.thumbnails)) return [];

  return [...videoDetails.thumbnails].sort((a, b) => {
 ...
 *
 * @param {Object} videoDetails - The `videoDetails` object from `ytdl.getInfo()`.
 * @param {'low' | 'medium' | 'high' | 'max'} resolution - Desired resolution level.
 *
 ...
 * @since 2.0.0
 */
function getThumbnailByResolution(videoDetails, resolution) {
  if (!resolution || !(['low', 'medium', 'high', 'max'].includes(resolution))) {
    throw new InvalidTypeError("Invalid resolution type. Use 'low', 'medium', 'high', or 'max'", {
      actualType: typeof resolution === 'string' ? resolution : TypeUtils.getType(resolution),
      expectedType: "'low' | 'medium' | 'high' | 'max'"
    });
  }

  // Get the sorted thumbnails for easier indexing
  const sortedThumbnails = getAllThumbnails(videoDetails);
  if (!sortedThumbnails.length) {
    throw new Error('Thumbnails are unavailable or cannot be found');
  }
  ...
  const targetKeys = resolutionMapping[resolution];
  ...
  return sortedThumbnails.find((thumb) => thumb.url.includes(resolutionMapping[resolution])) || null;
}
