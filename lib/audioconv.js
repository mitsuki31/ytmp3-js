 ...
 * @property {ffmpeg.FfprobeData} input - The input audio file information.
 ...
 * @property {Object} input.metadata - The metadata information of the input audio file.
  ...
  return await new Promise((resolve) => {
    const childProc = childProcess.exec('ffmpeg -version', (err) => {
      if (err) {
        ...
        verbose && console.error(`${err.name}: ${err.message}`);
        resolve(false);
      } else {
        verbose && log.done('FFmpeg is installed on the system');
        resolve(true);
      }
    });
    process.on('SIGINT', () => {
 ...
 *
 * @throws {Error} If ffmpeg fails to retrieve the metadata.
 *
 * @private
 * @since   2.0.0
 */
 ...
 */
async function convertAudio(inFile, outFile, options = defaultOptions) {
  /**
  ...
  const { resolveOptions, _AudioConverterOptions, _FFmpegCommandOptions, TypeUtils } = require('./utils');
  ...
  process.on('SIGINT', conversionInterruptedHandler, { once: true });

  /**
   * @ignore
   * @type {ResolvedConvertAudioOptions}
   */
  const convOptions = resolveOptions(options, _AudioConverterOptions);
  ...
  outFile = typeof outFile === 'string' ? path.resolve(outFile) : path.join(
    path.dirname(inFile),
    `${path.basename(inFile).replace(/\.[^/.]+$/, '')}.${(convOptions.format
      || (() => {
        let index = 0;
        if (Array.isArray(convOptions.outputOptions)
            && convOptions.outputOptions.length > 0) {
          options.outputconverterOptions.forEach((opt, idx) => {
            if (opt.match(/^-(acodec|c:a)/)) index = idx;
          });
          return options.outputconverterOptions[index].split(' ')[1];
        }
        return null;
      })()
  ...
  if (inFile === outFile) {
    ...
    if (ioBaseFile[1].match(/ \(copy(_[0-9]+)?\)$/)) {
      const copyNum = ioBaseFile[1].match(/_[0-9]+$/);
      ioBaseFile[1] = ioBaseFile[1].replace(/ \(copy(_[0-9]+)?\)$/, '');
      ioBaseFile[1] = ioBaseFile[1] + (copyNum ? `_${parseInt(copyNum[0].replace('_', '')) + 1}` : '_1');
    } else {
  ...
  await new Promise((resolve, reject) => {
    ...
    ffmpegChain
      .on('error', (err) => {
        ...
        quiet || log.error(`audioconv: ${err.message?.replace('\n', '')}`);
        quiet || log.error(`Input Audio: \x1b[33m${path.basename(inFile)}\x1b[0m `
