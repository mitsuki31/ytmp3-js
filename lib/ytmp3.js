...
/**
 * Retrieves information about multiple YouTube videos.
 *
...
async function getVideosInfo(...urls) {
  ...
  return await Promise.all(urls.map(async (url) => {
    ...
    return await ytdl.getInfo(url);  // Return the video info
  }));
}
...
async function* downloadAudio(...urls) {
  ...
  for await (const info of getVideosInfo(...urls)) {
    /**
     * @ignore
     * @type {VideoData}
     */
...
async function singleDownload(inputUrl) {
  inputUrl = inputUrl?.trim();  // Trim any whitespace
...
async function batchDownload(inputFile) {
  ...
  for (let next = await gen.next(); !next.done; next = await gen.next()) {
    ...
    const outFile = `${videoData.title}.m4a`;
    // Create a write stream for the output file
  ...
  allDownloads.forEach((title) => {
    ...
    console.log((downloaded ? '\x1b[2m' : '\x1b[1m') +
      `  [${downloaded ? '\u2714' : ' '}] ${(downloaded ? '' : '\x1b[93m') + title}\x1b[0m`);
  });
