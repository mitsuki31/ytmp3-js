 ...
 * It extends the `ytdl.downloadOptions` interface from the `ytdl-core` module, this way you can pass any option
 * of the `ytdl.downloadOptions` object.
 *
 ...
 * @property {Function} [handler] - A function to handle and customize the download process.
...
async function download(url, options) {
  async function isDownloaded(path, vInfo) {
    let res = false;
    try {
      res = res || (await fs.promises.stat(path)).isFile();
    // eslint-disable-next-line no-unused-vars
    } catch (_) {
      res = false;
    }
    ...
    await new Promise((resolve) => {
      require('fluent-ffmpeg').ffprobe(path, (err, meta) => {
        res = res || err ? false
          : Math.floor(parseInt(meta.format.duration)) === Math.floor(
   ...
   * @private
   */
  function downloadInterruptedHandler() {
  ...
  async function convertDownloadedAudio(path, options) {
    let result = null;
    try {
  ...
  process.on('SIGINT', downloadInterruptedHandler, { once: true });

  // Get the video information
  let videoInfo;
  let videoFormat;
  try {
    videoInfo = await getInfo(url, resolvedInfoOptions);
    videoFormat = FormatUtils.parseFormatObject(
  ...
  outFile = (outFile.trim().length && typeof outFile === 'string')
    ? outFile.trim()
    : videoInfo.videoDetails.title + '.m4a';
  outFile = !/.+\.\w+$/.test(outFile) ? `${outFile}.m4a` : outFile;  // Resolve extension file
  outFile = outFile.replace(INVALID_CHARS_REGEX, '_');  // Sanitize the output file name
  ...
  outStream.on('error', function errHandler(err) {
    quiet
      || log.error(`I/O error: Unable to write to file (${err.errno}): ${outFile}`);
    // Delete the file if there is no byte written yet
    if (fs.statSync(outStream.path).size === 0) fs.unlinkSync(outStream.path);
    throw err;
  });
  ...
  const data = {
    ...
    videoFormat,
    outStream,
    range: resolvedDlOptions.range,
    title: InfoUtils.getTitle(videoInfo),
    authorName: authorInfo.name,
    videoUrl: videoInfo.videoDetails.video_url,
    videoId: videoInfo.videoDetails.videoId,
    channelId: videoInfo.videoDetails.channelId,
    duration: InfoUtils.getDuration(videoInfo),
    viewers: InfoUtils.getViews(videoInfo),
    subscribers: InfoUtils.getSubscribers(videoInfo)
  };
    ...
    if (!quiet) {
      if (TypeUtils.isPlainObject(range) && typeof range.start === 'number') {
        ...
        log.info(`{\x1b[36m${videoId}\x1b[0m}: `
          + `Resume downloading from bytes [\x1b[96m${range.start} B//${mb} MB\x1b[0m]`);
      } else {
  ...
  process.off('SIGINT', downloadInterruptedHandler);
  ...
  const downloadResult = {
    ...
    url,
    cache: {
      useCache: resolvedDlOptions.useCache,
      id: resolvedDlOptions.useCache ? URLUtils.extractVideoId(url) : null,
      path: resolvedDlOptions.useCache ? getCachePath(data.videoId) : null
    },
    metadata: {
      title: data.title,
      description: InfoUtils.getDescription(videoInfo),
      publishDate: InfoUtils.getPublishDate(videoInfo),
      ...
      keywords: InfoUtils.getKeywords(videoInfo)
    },
    thumbnails: {
      author: ThumbnailUtils.sortThumbnailsByResolution(authorInfo.thumbnails),
      video: ThumbnailUtils.getVideoThumbnails(videoInfo.videoDetails, true)
    },
    ...
    downloadResult.conversionResult = await convertDownloadedAudio(output, resolveOptions(
      {
        ...
        quiet: allQuiet ? true : resolvedDlOptions.converterOptions.quiet
      },
