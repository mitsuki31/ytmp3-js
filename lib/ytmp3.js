 ...
 *          A promise fullfills with a video information. If the `url` is an array, returned value
 ...
 * @public
 * @since   2.0.0
 * @see     {@link module:cache~VINFO_CACHE_PATH}
 */
async function getInfo(url, options={}) {
  ...
  for (const [u, id] of urls.map((u, index) => [u, videoIDs[index]])) {
    ...
    const cache = useCache ? await VInfoCache.getCache(id) : null;
    let info = cache ? cache.videoInfo : null;
    // If the video information is not found in cache, fetch it from YouTube server instead
    if (!info) info = await ytdl.getInfo(u, options);
    else haveCache = true;  // Current video ID has cache
